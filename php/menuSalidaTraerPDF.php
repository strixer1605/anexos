    ...
    if (isset($idSalida)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($result->num_rows > 0) {
                ...
                $sqlAnexoX = "SELECT fkAnexoIV FROM anexox WHERE fkAnexoIV = ?";
                $stmtAnexoX = $conexion->prepare($sqlAnexoX);
                $stmtAnexoX->bind_param('i', $idSalida);
                $stmtAnexoX->execute();
                $resultAnexoX = $stmtAnexoX->get_result();
                
                if ($resultAnexoX->num_rows > 0) {
                    echo '<li><a href="../pdf/plantillaAnexoX.php" class="btn form-control botones w-100 mb-3">Anexo 10</a></li>';
                } else {
                    echo '<li><a class="btn form-control botones w-100 mb-3" disabled>Anexo 10 (Sin completar)</a></li>';
                }
                $stmtAnexoX->close();
